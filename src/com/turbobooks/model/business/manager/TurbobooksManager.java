 ...
 *
 */
public class TurbobooksManager extends ManagerSuperType {

	private static TurbobooksManager _instance;
	 ...
	 */
	private TurbobooksManager() {
		// construct object . . .
	}

	/**
	 * Assures that there is only one TurbobooksManager created.
	 * 
	 * @return TurbobooksManager instance
	 */
	public static synchronized TurbobooksManager getInstance() {
		if (_instance == null) {
			_instance = new TurbobooksManager();
		}
		return _instance;
	}
	...
	public boolean addItem(String commandString, Item item) {
		...
		try {
			iItemManagerService = (IItemManagerService) serviceFactory.getService(commandString);
			isAdded = iItemManagerService.addItem(item);
		} catch (ServiceLoadException e1) {
			log.error("TurbobooksManager::addItem failed " + e1);
	...
	public boolean removeItem(String commandString, Item item) {
		...
		try {
			iItemManagerService = (IItemManagerService) serviceFactory.getService(commandString);
			isRemoved = iItemManagerService.removeItem(item);
		} catch (ServiceLoadException e1) {
			log.error("TurbobooksManager::removeItem failed " + e1);
		} catch (ItemException e) {
	...
	public Item getItem(String commandString, String uuid) {
		...
		try {
			iItemManagerService = (IItemManagerService) serviceFactory.getService(commandString);
			return iItemManagerService.getItem(uuid);
		} catch (ServiceLoadException e1) {
			log.error("TurbobooksManager::getItem failed " + e1);
		} catch (ItemException e) {
	...
	public boolean updateItem(String commandString, Item item) {
		...
		try {
			iItemManagerService = (IItemManagerService) serviceFactory.getService(commandString);
			isUpdated = iItemManagerService.updateItem(item);
		} catch (ServiceLoadException e1) {
			log.error("TurbobooksManager::updateItem failed " + e1);
		} catch (ItemException e) {
	...
	public boolean checkoutItem(final String commandString, final String uuid, final String memberId) {
		...
		try {
			checkoutService = (ICheckoutService) serviceFactory.getService(commandString);
			isCheckedOut = checkoutService.checkoutItem(memberId, uuid);
		} catch (ServiceLoadException e1) {
			log.error("TurbobooksManager::checkoutItem failed " + e1);
	...
	public boolean isItemAvailable(final String commandString, final String uuid) {
		...
		try {
			...
			log.error("TurbobooksManager::isItemAvailable failed " + e1);
		} catch (CheckoutException e) {
